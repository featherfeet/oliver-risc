...
int main(int argc, char *argv[]) {
	...
	*******************************************************************************************************/
	...
	while (getline(&line, &line_length, input_file) != -1) {
		...
		*********************************************************************/
			...
			symbol_table[symbol_table_length - 1].value = (uint8_t) atoi(raw_symbol_value);
			free(cleaned_line);
			free(symbol_name);
	...
	*******************************************************************************************/
	void *symbol_table_binary = malloc(symbol_table_length);
	for (int i = 0; i < symbol_table_length; i++) {
		// Copy the 1-byte value from the symbol table into the binary.
		memcpy(symbol_table_binary + i, &symbol_table[i].value, 1);
	}
	...
	*******************************************************************************************/
	void *instruction_table_binary = malloc(instruction_table_length * 3);
	struct Instruction instruction;
	for (int i = 0; i < instruction_table_length; i++) {
		// Get the instruction.
		instruction = instruction_table[i];
		// Each instruction takes 3 bytes. Copy the operation to the binary.
		memcpy(instruction_table_binary + 3 * i, &instruction.operation, 1);
		...
		if (operand1_is_register) {
			memcpy(instruction_table_binary + 3 * i + 1, &instruction.operand1_register, 1);
		}
			...
			uint8_t binary_instruction_index = 3 * instruction_index;                                         // The address (in the binary .code section) of the instruction to jump to.
			memcpy(instruction_table_binary + 3 * i + 1, &binary_instruction_index, 1);
		}
			...
			memcpy(instruction_table_binary + 3 * i + 1, &symbol_location, 1);
		}
		if (operand2_is_register) {
			memcpy(instruction_table_binary + 3 * i + 2, &instruction.operand2_register, 1);
		}
			...
			memcpy(instruction_table_binary + 3 * i + 2, &symbol_location, 1);
		}
	}
	...
	*******************************************************************************************/
	...
	if (fwrite(symbol_table_binary, sizeof(uint8_t), symbol_table_length, output_file) != symbol_table_length) {
		printf("\033[1;31mERROR:\033[0m Could not write %d bytes of data to the file.\n", symbol_table_length);
	}
	uint8_t section_separator = (uint8_t) OPERATION_CODE;
	if (fwrite(&section_separator, sizeof(uint8_t), 1, output_file) != 1) {
		printf("\033[1;31mERROR:\033[0m Could not write 1 byte of data to the file.\n");
	}
	if (fwrite(instruction_table_binary, sizeof(uint8_t), instruction_table_length * 3, output_file) != instruction_table_length * 3) {
		printf("\033[1;31mERROR:\033[0m Could not write %d bytes of data to the file.\n", instruction_table_length * 3);
	}
	return 0;
}
